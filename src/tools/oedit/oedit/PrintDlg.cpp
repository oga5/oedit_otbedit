...
BOOL CPrintDlg::CreatePrnInfo()
{
	...
	// メモリ確保
	m_prninfo = (_PRINTER_INFO_2W *)calloc(m_prninfo_size, 1);
	if(m_prninfo == NULL) {
		goto ERR1;
	}

	// プリンタ情報を取得
	if(GetPrinter(hPrinter, 2, (LPBYTE)m_prninfo, m_prninfo_size, &dwNeeded) == FALSE) {
		goto ERR1;
	}
...
void CPrintDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_PRINT_SELECTED_AREA, m_print_selected_area);
	//}}AFX_DATA_MAP
}
...
BOOL CPrintDlg::OnInitDialog() 
{
	...
	if(m_backup_data.printer_name != NULL) {
		...
		m_printer_name = GetOWinApp()->GetIniFileString(_T("PRINT"), _T("PRINTER_NAME"), _T(""));
		if(m_printer_name.IsEmpty()) {
...
void CPrintDlg::OnOK()
{
	...
	pApp->WriteIniFileString(_T("PRINT"), _T("PRINTER_NAME"), m_printer_name);
	pApp->SaveIniFile();
...
void CPrintDlg::LoadPrintOption()
{
	...
	m_space_top = pApp->CWinApp::GetProfileInt(_T("PRINT"), _T("SPACE_TOP"), 20);
	m_space_bottom = pApp->CWinApp::GetProfileInt(_T("PRINT"), _T("SPACE_BOTTOM"), 15);
	m_space_left = pApp->CWinApp::GetProfileInt(_T("PRINT"), _T("SPACE_LEFT"), 15);
	m_space_right = pApp->CWinApp::GetProfileInt(_T("PRINT"), _T("SPACE_RIGHT"), 15);
	m_print_title = pApp->CWinApp::GetProfileInt(_T("PRINT"), _T("PRINT_TITLE"), 1);
	m_print_page = pApp->CWinApp::GetProfileInt(_T("PRINT"), _T("PRINT_PAGE"), FALSE);
	m_print_row_num = pApp->CWinApp::GetProfileInt(_T("PRINT"), _T("PRINT_ROW_NUM"), FALSE);
	m_print_date = pApp->CWinApp::GetProfileInt(_T("PRINT"), _T("PRINT_DATE"), FALSE);

	m_font_face_name = AfxGetApp()->CWinApp::GetProfileString(_T("PRINT"), _T("FONT_FACE_NAME"), _T("ＭＳ ゴシック"));
	m_font_point = AfxGetApp()->CWinApp::GetProfileInt(_T("PRINT"), _T("FONT_POINT"), 110);
}
...
void CPrintDlg::SavePrintOption()
{
	...
	pApp->CWinApp::WriteProfileInt(_T("PRINT"), _T("SPACE_TOP"), m_space_top);
	pApp->CWinApp::WriteProfileInt(_T("PRINT"), _T("SPACE_BOTTOM"), m_space_bottom);
	pApp->CWinApp::WriteProfileInt(_T("PRINT"), _T("SPACE_LEFT"), m_space_left);
	pApp->CWinApp::WriteProfileInt(_T("PRINT"), _T("SPACE_RIGHT"), m_space_right);
	pApp->CWinApp::WriteProfileInt(_T("PRINT"), _T("PRINT_TITLE"), m_print_title);
	pApp->CWinApp::WriteProfileInt(_T("PRINT"), _T("PRINT_PAGE"), m_print_page);
	pApp->CWinApp::WriteProfileInt(_T("PRINT"), _T("PRINT_ROW_NUM"), m_print_row_num);
	pApp->CWinApp::WriteProfileInt(_T("PRINT"), _T("PRINT_DATE"), m_print_date);

	pApp->CWinApp::WriteProfileString(_T("PRINT"), _T("FONT_FACE_NAME"), m_font_face_name);
	pApp->CWinApp::WriteProfileInt(_T("PRINT"), _T("FONT_POINT"), m_font_point);
}
...
void CPrintDlg::CheckBtn()
{
	...
	if(m_prninfo != NULL && m_prninfo->pDevMode != NULL && printer_dc != NULL) {
